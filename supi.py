...
def time(sec):
	tm.sleep(sec)
def curl(keyword,headers):
	return req.get('https://shopee.co.id/api/v2/search_items/?by=relevancy&keyword='+keyword+'&limit=100&newest=0&order=desc&page_type=search&version=2',headers=headers)
...
try:
	...
	print(" Searching "+ keyword + " on shopee")
	p = curl(keyword,headers)
